function SRExperiment(subjectNumber)
...
try
    ...
    for block = 1:(numberOfBlocks)
        ...
        if use_gamepad == 0
            message = sprintf('BLOCK %d OUT OF %d\n\n You have %d seconds to complete trial. \n\n\nPress mouse button to start experiment.', block, numberOfBlocks, T);
        else
            message = sprintf('CC SEARCH TASK - BLOCK %d OUT OF %d\n\nPress Button %d or Button %d for target (letter T) pointing to the left or right, resp.\n\n\nPress mouse button to start experiment.', block-1, numberOfBlocks, left_button, right_button);
        end
        ...
        % loop through trials
        trial = 1;
        while (trial <= displaysPerBlock && ~KbCheck)
            ...
            message1 = sprintf('Brief Rest Period.\n\n Trial Number %d Out Of %d Cleared \\n Block %d ', trial , displaysPerBlock, block);
            DrawFormattedText(w, message1, 'center', 'center', WhiteIndex(w));
            Screen('Flip', w);
            WaitSecs(0.5);
            FlushEvents('keyDown');
...
%                 end
            ...
            fprintf(datafilepointer, '%02d\t%d\t%d\t%d\t%s\t%s\t%s\t%s\t%d\n', ...
                subjectNumber, (block-1), trial, trialOrder(trial), displayString, tiltString, KeyCode, answerstring, rt);
            trial = trial + 1;
        end
    end
